...
MAX_REPORT_ITEMS = 8
MIN_COMPLETE_ITEMS = 6
MIN_COMPLETE_SECTIONS = 2
MIN_COMPLETE_IMAGES = 1
...
class NativeReportPipeline:
    ...
    async def run(
        ...
        primary_window_start = window_end - timedelta(
            hours=PRIMARY_RETRIEVAL_WINDOW_HOURS
        )
        extended_window_start = window_end - timedelta(
            hours=EXTENDED_RETRIEVAL_WINDOW_HOURS
        )
        run = RetrievalRun(
        ...
        try:
            sources = list(
                session.scalars(
                    ...
                    .order_by(desc(Source.priority))
                ).all()
            )
            feedback_stats = quality_feedback_domain_stats(session)

            planner_started = perf_counter()
            query_specs, priority_domains, planner_meta = await self._plan_queries(
                target_date, sources, runtime
            )
            stage_durations["planner_seconds"] = round(
                perf_counter() - planner_started, 3
            )
            ...
            stage_durations["discovery_seconds"] = round(
                perf_counter() - discovery_started, 3
            )
            ...
            stage_durations["extraction_seconds"] = round(
                perf_counter() - extraction_started, 3
            )
            ...
            stage_durations["scorer_seconds"] = round(
                perf_counter() - scoring_started, 3
            )
            ...
            (
                ...
                report_items,
                writer_meta,
            ) = await self._build_report_content(
                session,
                run,
                scored_articles,
                target_date,
            ...
            stage_durations["writer_seconds"] = round(
                perf_counter() - writer_started, 3
            )
            ...
            off_topic_rejections = rejection_counts.get(
                "off_topic_candidate", 0
            ) + rejection_counts.get("off_topic_content", 0)
            pr_like_rejections = rejection_counts.get(
                "pr_like_candidate", 0
            ) + rejection_counts.get("pr_like_content", 0)
            feedback_hits = self._feedback_hits(session, run.id, feedback_stats)
            section_coverage = distinct_sections(
                item["section"] for item in report_items
            )
            high_confidence_count = sum(
                1 for article in scored_articles if self._is_high_confidence(article)
            )
            window_bucket_counts = self._window_bucket_counts(articles)
            extended_window_selected = sum(
                1
                for item in report_items
                if item.get("window_bucket") == EXTENDED_WINDOW_BUCKET
            )
            metadata_fallback_count = sum(
                1
                for article in articles
                if article.extraction_status == "search_fallback"
            )
            policy_candidate_count = section_candidate_counts.get("policy", 0)
            policy_selected_count = sum(
                1 for item in report_items if item["section"] == "policy"
            )
            per_domain_selected = self._per_domain_selected(report_items)
            top_policy_misses = self._top_policy_misses(
                rejection_counts, section_candidate_counts, policy_selected_count
            )
            operational_rejections = {
                reason: count
                for reason, count in rejection_counts.items()
                if reason.startswith("scrape_error")
                or reason in {"domain_circuit_breaker"}
            }
            if status in {"complete", "partial"} and (
                fallbacks_triggered
                or provider_errors
                or operational_rejections
                or query_error_count
            ):
                status = "degraded"
            ...
            report = Report(
                ...
                summary=self._compose_report_summary(
                    report_items, len(articles), rejection_counts, base_summary=summary
                ),
    ...
    async def _plan_queries(
        ...
        planner_output, meta = await self.llm.plan_queries(
            target_date, source_payload, SECTION_META, runtime
        )
        ...
        explicit_priority_domains = [
            domain
            for domain in planner_output.priority_domains
            if domain in {source.domain for source in sources}
        ]
        source_priority_domains = [
            source.domain
            for source in sources
            if source.priority >= 80 or source.use_direct_source
        ]
        priority_domains = list(
            dict.fromkeys(explicit_priority_domains + source_priority_domains)
        )
    ...
    async def _discover_candidates(
        ...
        for spec in query_specs:
            ...
            for row in results:
                source = self._match_source_for_domain(
                    row.get("domain") or extract_domain(row["url"]), source_lookup
                )
                candidate = self._candidate_from_search_result(
                    row, spec["section"], spec["language"], source=source
                )
                persisted = self._persist_candidate(
                    session, run.id, query_row.id, candidate
                )
                rejection_reason = self._prefilter_candidate(
                    persisted,
                    source,
                    primary_window_start,
                    extended_window_start,
    ...
    def _default_query_specs(self) -> list[dict[str, str]]:
        ...
        for section, meta in SECTION_META.items():
            for language, query in meta["queries"]:
                if (
                    counts_by_section[section] >= SECTION_QUERY_LIMITS.get(section, 0)
                    or len(query_specs) >= MAX_QUERIES_PER_RUN
                ):
                    break
                query_specs.append(
                    {
    ...
    def _candidate_from_search_result(
        ...
        default_section = (
            SOURCE_TYPE_TO_SECTION.get(source.type, section) if source else section
        )
        classified_section = self._classify_section(
            result["title"],
            result.get("snippet"),
            source.type if source else section,
            default_section,
        )
        ...
        for source in source_order:
            ...
            try:
                if source.crawl_mode == "rss":
                    feed_entries = await fetch_feed_entries(
                        source.rss_or_listing_url, source.name, source.type
                    )
                    for entry in feed_entries:
                        if not self._passes_source_rules(
                            entry["title"], entry.get("snippet"), source
                        ):
                            continue
                        if entry.get("published_at") and not self._is_recent(
                            entry["published_at"], window_start, window_end
                        ):
                            continue
                        direct_candidates.append(
                            {
                                ...
                                "title": entry["title"],
                                "domain": entry["domain"],
                                "section": SOURCE_TYPE_TO_SECTION.get(
                                    source.type, "industry"
                                ),
                                "language": source.language
                                or infer_language(entry["title"], entry.get("snippet")),
                                ...
                                "metadata": {
                                    **(entry.get("metadata") or {}),
                                    "source_tier": self._resolve_source_tier(
                    ...
                    for link in links[:8]:
                        mapped = self._normalize_listing_map_entry(
                            link, source.rss_or_listing_url
                        )
                        if mapped is None:
                            continue
                        direct_candidates.append(
    ...
    def _normalize_listing_map_entry(
        self, link: Any, listing_url: str
    ) -> dict[str, Any] | None:
        if isinstance(link, str):
            ...
            snippet = (
                str(link.get("description") or link.get("snippet") or "").strip()
                or None
            )
    ...
    def _is_listing_like_url(self, url: str, title: str | None = None) -> bool:
        ...
        if any(
            ...
            for token in ["/page/", "sitemap", ".xml", "/tag/", "/category/"]
        ):
            return True
        if any(
            token in lowered_title
            for token in ["archives", "archive", "样本", "sample"]
        ):
            return True
        return False
    ...
    def _deduplicate_candidates(
        ...
        ranked_candidates = sorted(
            candidates, key=self._candidate_rank_key, reverse=True
        )
    ...
    async def _extract_articles(
        ...
        for candidate in prioritized:
            if (
                extraction_domain_counts[candidate["domain"]]
                >= PER_DOMAIN_EXTRACTION_CAP
            ):
                row = candidate.get("candidate_row")
        ...
        for candidate in selected:
            ...
            if (
                failure_counts[candidate["domain"]]
                >= runtime["domain_failure_threshold"]
            ):
                self._reject_candidate(candidate, "domain_circuit_breaker")
                continue
            if source and not self._passes_source_rules(
                candidate["title"], candidate.get("snippet"), source
            ):
    ...
    async def _process_extraction_batch(
        ...
        results = await asyncio.gather(
            *[
                self.scraper.scrape(
                    candidate["url"], timeout_seconds=runtime["scrape_timeout_seconds"]
                )
        ...
        for candidate, result in zip(batch, results, strict=False):
            ...
            if isinstance(result, Exception):
                ...
                if fallback_article is None:
                    self._reject_candidate(
                        candidate, f"scrape_error:{type(result).__name__}"
                    )
                    continue
            ...
            if published_at is None:
                ...
                if source_tier in ALLOW_MISSING_PUBLISHED_AT_TIERS:
                    extracted_date = self._extract_policy_date(
                        markdown, candidate["domain"]
                    )
                    if extracted_date:
            ...
            article = Article(
                ...
                language=infer_language(
                    candidate.get("title"), enriched_markdown, candidate.get("snippet")
                ),
                ...
                image_url=(
                    ...
                    if (source.allow_images if source else True)
                    else None
                )
                ...
                metadata_json={
                    ...
                    **(result.get("metadata") or {}),
                    "candidate_id": candidate.get("candidate_row").id
                    if candidate.get("candidate_row")
                    else None,
                    "source_tier": self._resolve_source_tier(
                        candidate["domain"], source
                    ),
                    "window_bucket": window_bucket
                    or candidate.get("metadata", {}).get("window_bucket")
                    or PRIMARY_WINDOW_BUCKET,
                },
            )
            self._score_article_heuristic(
                article,
                target_date,
                source.priority if source else 40,
                self._resolve_source_tier(candidate["domain"], source),
            )
            session.add(article)
            session.flush()
    ...
    def _build_search_fallback_article(
        ...
        resolved_source_type = source.type if source else candidate["source_type"]
        section = self._classify_section(
            title, fallback_markdown, resolved_source_type, candidate["section"]
        )
        ...
        article = Article(
            ...
            section=section,
            language=infer_language(
                candidate.get("title"), fallback_markdown, candidate.get("snippet")
            ),
            country=source.country if source else None,
            source_name=source.name if source else candidate["source_name"],
            published_at=published_at,
        ...
        self._score_article_heuristic(
            article, target_date, source.priority if source else 40, source_tier
        )
        return article
    ...
    def _extract_policy_date(self, markdown: str, domain: str) -> datetime | None:
        ...
        if (
            not any(domain.endswith(p) for p in policy_domains)
            and "government" not in domain
        ):
            return None
        ...
        date_patterns = [
            ...
            (
                r"发布[于在](\d{4})年(\d{1,2})月",
                lambda m: datetime(int(m[0]), int(m[1]), 1),
            ),
            (
            ...
            (
                r"发文日期[：:]\s*(\d{4})年(\d{1,2})月(\d{1,2})日",
                lambda m: datetime(int(m[0]), int(m[1]), int(m[2])),
            ),
            (
    ...
    def _merge_candidate_context(
        self, candidate: dict[str, Any], base_text: str | None
    ) -> str:
        metadata = candidate.get("metadata") or {}
        chunks: list[str] = []
    ...
    async def _score_articles(
        ...
        payload_articles = []
        for article in articles:
            matched_source = self._match_source_for_domain(
                article.domain, source_lookup
            )
            self._score_article_heuristic(
                article,
                target_date,
                source_priority.get(
                    matched_source.domain if matched_source else article.domain, 40
                ),
            ...
            payload_articles.append(
                {
                    ...
                    "source_tier": self._resolve_source_tier(
                        article.domain, matched_source, article
                    ),
                }
            )

        llm_output, meta = await self.llm.score_articles(
            target_date, payload_articles, runtime
        )
        ...
        for article in articles:
            ...
            article.section = (
                decision.section
                if decision.section in SECTION_META
                else article.section
            )
            article.freshness_score = self._clamp_score(
                decision.freshness_score, article.freshness_score
            )
            article.relevance_score = self._clamp_score(
                decision.relevance_score, article.relevance_score
            )
            article.source_trust_score = self._clamp_score(
                decision.source_trust_score, article.source_trust_score
            )
            article.research_value_score = self._clamp_score(
                decision.research_value_score, article.research_value_score
            )
            article.novelty_score = self._clamp_score(
                decision.novelty_score, article.novelty_score
            )
            article.combined_score = self._clamp_score(
                decision.combined_score, article.combined_score
            )
            article.metadata_json = {
                **(article.metadata_json or {}),
                ...
                or self._research_signal(article),
            }
    ...
    def _score_article_heuristic(
        ...
        positive_hits = sum(
            1 for keyword in GLOBAL_TOPIC_TERMS if keyword.lower() in content
        )
        section_hits = sum(
            1
            for keyword in {
        ...
        negative_hits = sum(
            1 for keyword in OFF_TOPIC_TERMS if keyword.lower() in content
        )
        soft_reject_hits = sum(
            1 for keyword in SOFT_REJECT_TERMS if keyword.lower() in content
        )
        ...
        relevance = max(
            0.0,
            relevance - negative_hits * 0.28 - pr_hits * 0.2 - soft_reject_hits * 0.1,
        )
        ...
        if article.section == "academic":
            ...
            term in content for term in ["机理", "实验", "breakthrough", "mechanism"]
        ):
            research_value = 0.78
    ...
    def _select_articles_from_scores(self, articles: list[Article]) -> list[Article]:
        eligible = [
            ...
            for article in sorted(
                articles, key=lambda item: item.combined_score, reverse=True
            )
            if self._passes_final_quality_gate(article)
        ]
        if not eligible:
            return []
        ...
        primary_articles = [
            ...
            if self._article_window_bucket(article) == PRIMARY_WINDOW_BUCKET
        ]
        extended_articles = [
            article
            for article in eligible
            if self._article_window_bucket(article) == EXTENDED_WINDOW_BUCKET
            and self._resolve_source_tier(article.domain, article=article)
            in EXTENDED_WINDOW_TIERS
        ]
        ...
        if len(selected) < MIN_COMPLETE_ITEMS:
            ...
            for section in ("policy", "industry"):
                if (
                    extended_added >= MAX_EXTENDED_WINDOW_ITEMS
                    or len(selected) >= MAX_REPORT_ITEMS
                ):
                    break
            ...
            for article in extended_articles:
                if (
                    extended_added >= MAX_EXTENDED_WINDOW_ITEMS
                    or len(selected) >= MAX_REPORT_ITEMS
                    or len(selected) >= MIN_COMPLETE_ITEMS
                ):
                    break
                if try_append(article):
                    extended_added += 1
    ...
    async def _build_report_content(
        ...
        if not articles:
            summary = (
                "未命中足够候选文章，请检查 Brave、Jina、OpenRouter 配置和来源规则。"
            )
            markdown = "\n".join(
                [
        ...
        if writer_output is None:
            status, title, markdown, summary, items = (
                self._build_template_report_content(articles, target_date)
            )
        ...
        return (
            status,
            writer_output.title,
            writer_output.markdown_content,
                ...
                report_items.append(
                    {
                        ...
                        "citations": [
                            {
                                "label": article.source_name or article.domain,
                                "url": article.url,
                            }
                        ],
                        "combined_score": article.combined_score,
    ...
    def _hydrate_writer_items(
        self, writer_output: WriterOutput, article_map: dict[int, Article]
    ) -> list[dict[str, Any]]:
        report_items: list[dict[str, Any]] = []
        ...
        for row in sorted(writer_output.items, key=lambda item: item.rank):
            ...
            if (
                article is None
                or row.article_id in seen_ids
                or not self._passes_final_quality_gate(article)
            ):
                return []
            ...
            report_items.append(
                {
                    "article_id": article.id,
                    "section": row.section
                    if row.section in SECTION_META
                    else article.section,
                    "rank": row.rank,
                    "title": article.title,
                    "source_name": article.source_name or article.domain,
                    ...
                    or self._article_research_signal(article),
                    "image_url": article.image_url,
                    "window_bucket": self._article_window_bucket(article),
                    "citations": [
                        {
    ...
    def _research_signal(self, article: Article) -> str:
        ...
        return (
            "关注政策、标准或终端需求变化对材料选择、回收体系和合规测试带来的研究牵引。"
        )

    def _match_source_for_domain(
        self, domain: str, source_map: dict[str, Source]
    ) -> Source | None:
        normalized = extract_domain(
            domain if domain.startswith("http") else f"https://{domain}"
        )
        exact = source_map.get(normalized)
        if exact is not None:
            return exact
        for source_domain, source in sorted(
            source_map.items(), key=lambda item: len(item[0]), reverse=True
        ):
            if normalized == source_domain or normalized.endswith(f".{source_domain}"):
                return source
        return None

    def _source_priority_for_candidate(
        self, domain: str, source_map: dict[str, Source]
    ) -> int:
        source = self._match_source_for_domain(domain, source_map)
        return source.priority if source else 10

    def _resolve_source_tier(
        self, domain: str, source: Source | None = None, article: Article | None = None
    ) -> str:
    ...
    def _resolve_window_bucket(
        ...
        if source_tier in EXTENDED_WINDOW_TIERS and self._is_recent(
            published_local, extended_window_start, window_end
        ):
    ...
    def _prefilter_candidate(
        ...
        if (
            published_at
            and primary_window_start
            and extended_window_start
            and window_end
        ):
            window_bucket = self._resolve_window_bucket(
                published_at,
        ...
        if published_at is None and metadata.get("search_type") == "images":
        ...
        if not self._passes_source_rules(
            ...
            return "source_rules"
        if not self._passes_topic_gate(
            candidate["title"], candidate.get("snippet"), source
        ):
            return "off_topic_candidate"
        if self._is_pr_like(
            domain, content
        ) and not self._passes_strong_relevance_signal(content):
            return "pr_like_candidate"
        return None

    def _passes_topic_gate(
        self, title: str, text: str | None, source: Source | None
    ) -> bool:
        content = f"{title} {text or ''}".lower()
        if source:
            must_include = list(source.must_include_any or []) or list(
                source.include_rules or []
            )
            if must_include and not self._contains_any(content, must_include):
                if self._is_high_tier_source(
                    source.source_tier
                ) and self._passes_high_tier_exception(content, source.type):
                    return True
                return False
            return self._contains_any(
                content,
                GLOBAL_TOPIC_TERMS + list(source.soft_signals or []) + must_include,
            )
        return self._contains_any(content, GLOBAL_TOPIC_TERMS)

    def _passes_content_gate(
        self, title: str, text: str | None, source: Source | None, section: str
    ) -> bool:
        content = f"{title} {text or ''}".lower()
        if self._contains_any(content, OFF_TOPIC_TERMS):
            return False
        ...
        return self._contains_any(content, GLOBAL_TOPIC_TERMS) and (
            self._contains_any(content, section_terms)
            or self._passes_strong_relevance_signal(content)
        )
    ...
    def _passes_final_quality_gate(self, article: Article) -> bool:
        ...
        if self._contains_any(
            content, SOFT_REJECT_TERMS
        ) and article.combined_score < max(
        ...
        if (
            source_tier == "unknown"
            and article.combined_score < HIGH_CONFIDENCE_SCORE_THRESHOLD
        ):
            return False
        if not self._passes_content_gate(
            article.title,
            article.raw_markdown or article.summary,
            None,
            article.section,
        ):
            return False
    ...
    def _status_for_report_items(self, report_items: list[dict[str, Any]]) -> str:
        ...
        extended_used = any(
            item.get("window_bucket") == EXTENDED_WINDOW_BUCKET for item in report_items
        )
        verified_image_count = sum(
            1 for item in report_items if self._item_has_publishable_image(item)
        )
        average_score = sum(
            float(item.get("combined_score") or 0.0) for item in report_items
        ) / max(len(report_items), 1)
        minimum_score = min(
            float(item.get("combined_score") or 0.0) for item in report_items
        )
        has_quality_floor = (
            minimum_score >= QUALITY_SCORE_THRESHOLD
            and average_score >= QUALITY_SCORE_THRESHOLD
        )
        has_image_floor = verified_image_count >= MIN_COMPLETE_IMAGES
        # complete: 6+ items + 2+ sections + 3+ verified images + no extended + all scores pass floor
        if (
            len(report_items) >= MIN_COMPLETE_ITEMS
            and section_count >= MIN_COMPLETE_SECTIONS
            and has_image_floor
            and has_quality_floor
            and not extended_used
        ):
            return "complete"
        # partial: any items that don't meet complete criteria
        return "partial"

    def _partial_gap_description(
        self,
        report_items: list[dict[str, Any]],
        image_gap_reason: str | None = None,
        policy_gap_reason: str | None = None,
    ) -> str | None:
    ...
    def _item_has_publishable_image(self, item: dict[str, Any]) -> bool:
        if item.get("has_verified_image"):
            return True
        return bool(str(item.get("image_url") or "").strip())

    def _quality_gate_counts(
        self, rejection_counts: dict[str, IntProperty]
    ) -> dict[str, int]:
        return {
            ...
            "missing_published_at": rejection_counts.get("missing_published_at", 0)
            + rejection_counts.get("missing_published_at_candidate", 0),
            "off_topic": rejection_counts.get("off_topic_candidate", 0)
            + rejection_counts.get("off_topic_content", 0),
            "pr_like": rejection_counts.get("pr_like_candidate", 0)
            + rejection_counts.get("pr_like_content", 0),
            "source_rules": rejection_counts.get("source_rules", 0)
            + rejection_counts.get("source_rules_after_extract", 0),
    ...
    def _domain_penalties(
        ...
        stmt = select(
            RetrievalCandidate.domain, RetrievalCandidate.rejection_reason
        ...
        return dict(
            sorted(penalties.items(), key=lambda item: item[1], reverse=True)[:8]
        )
    ...
    def _feedback_hits(
        ...
        domains = {
            ...
            for domain in session.scalars(
                select(RetrievalCandidate.domain).where(
                    RetrievalCandidate.run_id == run_id
                )
            ).all()
            if domain
        ...
        return sum(
            ...
            if self._feedback_penalty_score(feedback_stats.get(domain, {})) > 0
        )
    ...
    def _section_candidate_counts(self, session, run_id: int) -> dict[str, int]:
        ...
        rows = session.execute(
            select(
                RetrievalCandidate.section, RetrievalCandidate.rejection_reason
            ).where(RetrievalCandidate.run_id == run_id)
        ).all()
        for section, reason in rows:
            if not section or reason in {
                "duplicate_url_or_title",
                "domain_candidate_cap",
            }:
                continue
            counts[section] += 1
        return dict(counts)

    def _section_selected_counts(
    ...
    def _source_rule_rejections(self, session, run_id: int) -> dict[str, int]:
        ...
        rows = session.execute(
            select(
                ...
                RetrievalCandidate.run_id == run_id,
                RetrievalCandidate.rejection_reason.in_(
                    ["source_rules", "source_rules_after_extract"]
                ),
            )
        ).all()
    ...
    def _classify_section(
        self, title: str, snippet: str | None, source_type: str, default: str
    ) -> str:
        content = f"{title} {snippet or ''}".lower()
        if source_type == "policy":
            return "policy"
        if source_type == "academic" and self._contains_any(
            content, GLOBAL_TOPIC_TERMS + ACADEMIC_TERMS
        ):
            return "academic"
        if self._contains_any(content, POLICY_TERMS):
            return "policy"
        if self._contains_any(content, ACADEMIC_TERMS) and self._contains_any(
            content, GLOBAL_TOPIC_TERMS
        ):
            return "academic"
        if self._contains_any(content, INDUSTRY_TERMS) and self._contains_any(
            content, GLOBAL_TOPIC_TERMS
        ):
            return "industry"
        return SOURCE_TYPE_TO_SECTION.get(source_type, default)

    def _passes_source_rules(
        self, title: str, text: str | None, source: Source | None
    ) -> bool:
        if source is None:
            return True
        content = f"{title} {text or ''}".lower()
        combined_excludes = list(source.must_exclude_any or []) + list(
            source.exclude_rules or []
        )
        combined_includes = list(source.must_include_any or []) + list(
            source.include_rules or []
        )
        if combined_excludes and self._contains_excluded_term(
            content, combined_excludes
        ):
            return False
        ...
        if self._is_high_tier_source(
            ...
            return True
        return self._contains_any(
            content, GLOBAL_TOPIC_TERMS + list(source.soft_signals or [])
        )

    def _is_recent(
        self, published_at: datetime, window_start: datetime, window_end: datetime
    ) -> bool:
        candidate = published_at.replace(tzinfo=None)
        return window_start <= candidate <= window_end

    def _persist_candidate(
        self, session, run_id: int, query_id: int | None, candidate: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _contains_excluded_term(self, content: str, keywords: list[str]) -> bool:
        for keyword in keywords:
            ...
            if lowered.isascii():
                pattern = (
                    r"(?<!\w)" + re.escape(lowered).replace(r"\ ", r"\s+") + r"(?!\w)"
                )
    ...
    def _is_pr_like(self, domain: str, content: str) -> bool:
        return any(
            pattern in domain for pattern in PR_WIRE_DOMAIN_PATTERNS
        ) or self._contains_any(content, PR_LIKE_TERMS)
    ...
    def _looks_like_verification_wall(
        self, title: str | None, content: str | None
    ) -> bool:
    ...
    def _candidate_rank_key(
        ...
        search_rank = {"direct": 3, "news": 2, "web": 1, "images": 0}.get(
            search_type, 0
        )
        return (
    ...
    def _is_high_confidence(self, article: Article) -> bool:
        return article.combined_score >= max(
            HIGH_CONFIDENCE_SCORE_THRESHOLD,
            self._quality_threshold_for_tier(
                self._resolve_source_tier(article.domain, article=article)
            )
            + 0.08,
        )
    ...
    def _feedback_penalty_score(self, counts: dict[str, int] | None) -> float:
        if not counts:
            return 0.0
        bad = (
            counts.get("bad_off_topic", 0) * 2
            + counts.get("bad_pr_like", 0) * 2
            + counts.get("bad_source", 0)
        )
