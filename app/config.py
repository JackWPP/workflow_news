...
class Settings:
    ...
    strict_primary_model_for_all_llm: bool = _as_bool(
        os.getenv("STRICT_PRIMARY_MODEL_FOR_ALL_LLM"), default=True
    )
    ...
    max_extractions_per_run: int = int(os.getenv("MAX_EXTRACTIONS_PER_RUN", "24"))
    max_items_per_section: int = int(os.getenv("MAX_ITEMS_PER_SECTION", "4"))
    report_min_formal_topics: int = int(os.getenv("REPORT_MIN_FORMAL_TOPICS", "4"))
    report_target_items: int = int(os.getenv("REPORT_TARGET_ITEMS", "6"))
    ...
    multi_agent_mode: bool = _as_bool(os.getenv("MULTI_AGENT_MODE"), default=True)
